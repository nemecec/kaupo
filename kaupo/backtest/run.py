...
async def run_backtest(
    ...
    async with session_scope() as session:
        candles = await get_candles(session, request.pair, request.timeframe, request.start, request.end)
    if not candles:
        raise ValueError(
            f"No candles for {request.pair} {request.timeframe.value} in range; run `kaupo ingest` first"
        )
    log.info("Backtesting %s on %d candles", request.strategy.id, len(candles))
    ...
    result = await engine.run(_aiter(candles))
