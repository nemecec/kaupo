#!/usr/bin/env bash
# Refresh the Kraken rolling windows for the pair-quality universe.
...
# without a regular re-ingest. Runs daily from cron on the host. Idempotent
# (upserts).
set -uo pipefail
...
[[ "$fail" -eq 0 ]] && echo "refreshed kraken ${TIMEFRAMES} for ${PAIRS}"
exit "$fail"
